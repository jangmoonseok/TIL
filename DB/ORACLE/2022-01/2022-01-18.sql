 -- 2022-0118-1)---------------------------------------------------------------------------------------------------------------
    ...
    TABLE JOIN
     - 데이터베이스 설계에 정규화 과정을 수행하면 테이블이 분할되고 필요한 자료를 조회하기위해 복수개의 테이블이 공통의 컬럼을 기준으로 연산에 참여 해야함
       => 조인연산
     - 관계형 데이터베이스의 기본연산
     - 종류
      . 내부조인(INNER JOIN)과 외부조인(OUTER JOIN) -- 내부조인은 데이터의 종류가 많으면 데이터가 적은쪽을 기준으로 남는 데이터는 무시 <=> 외부조인 
      . 일반조인과 ANSI JOIN
    ...
    1. Cartesian Product
     ...
     - 조인의 결과는 최악의 경우 행의 수를 곱한 결과와 열의 수를 더한 결과 반환
     - 불가피하게 필요한 경우가 아니면 사용하지 않는다.
     (사용형식)
     SELECT 컬럼list
 ...
 -- 2022-0118-2)---------------------------------------------------------------------------------------------------------------
    ...
    2. Equi JOIN
     - 조인조건문에 동등연산자('=')가 사용
     - 사용된 테이블의 수가 N개일때 조인조건은 적어도 N-1개 이상 이어야함
     - ANSI에서는 INNER JOIN 사용을 권고함
     (기술형식 - 일반 조인문)
     SELECT [테이블명.|테이블별칠.]컬럼명 [AS 컬럼별칭]
                        :
     ...
     [WHERE 일반조건];
      - 테이블명1과 테이블명2는 반드시 조인 가능해야함 -- 공통컬럼이 있어야함
     
     사용예)
        상품테이블과 분류테이블을 이용하여 판매가가 10만원 이상인 상품을 조회하시오
    ...
    사용예)
        ...
        SELECT A.MEM_ID AS 회원번호,
         ...
         WHERE A.MEM_ID = B.CART_MEMBER -- 조인조건 : 회원번호 추출
           AND B.CART_PROD = C.PROD_ID -- 조인조건 : 판매단가 추출
         ...
         INNER JOIN PROD C ON(C.PROD_ID = B.CART_PROD AND
                              B.CART_NO LIKE '200506%')
 ...
 -- 2022-0118-3)---------------------------------------------------------------------------------------------------------------        
    ...
    사용예)
        부서별 인원수와 평균임금을 조회하시오
        Alias 는 부서코드, 부서명, 인원수, 평균임금
    ...
    문제]
        2005년 1월 ~ 6월 각 거래처별 매입현황을 조회하시오
        ...
        SELECT A.BUYER_ID AS 거래처코드,
         ...
         INNER JOIN BUYPROD C ON(B.PROD_ID = C.BUY_PROD AND
                                 C.BUY_DATE BETWEEN TO_DATE('20050101') AND TO_DATE('20050630'))
         GROUP BY A.BUYER_ID, A.BUYER_NAME
         ORDER BY 1;
 ...
 -- 2022-0118-4)---------------------------------------------------------------------------------------------------------------         
    ...
    문제]
     ...
     문제]
        2005년 4월~6월 각 상품별 매입/매출현황을 조회하시오.
        ...
        SELECT A.PROD_ID AS 상품코드,
               ...
               C.CSUM AS 매출금액합계
          FROM PROD A,
               (SELECT AA.PROD_ID AS BID,
                       SUM(BB.BUY_QTY * AA.PROD_COST) AS BSUM
                  FROM PROD AA, BUYPROD BB
                 WHERE AA.PROD_ID = BB.BUY_PROD
                   AND BB.BUY_DATE BETWEEN TO_DATE('20050401') AND TO_DATE('20050630')
                 GROUP BY AA.PROD_ID) B,
               (SELECT BB.PROD_ID AS CID,
