...
![image](https://user-images.githubusercontent.com/64582209/156316916-74d8e27b-e0f4-4de2-9aaf-8808bb876021.png)
...
class Solution {
    public int solution(int[][] board, int[] moves) {
        ...
        Map<Integer, List<Integer>> boardMap  = new HashMap<Integer, List<Integer>>();
        for(int i = 0; i < board.length; i++){
            boardMap.put(i + 1, new ArrayList<Integer>());
            List<Integer> temp = boardMap.get(i + 1);
        ...
        for(int crain : moves){
            List<Integer> col = boardMap.get(crain);
            for(int i = 0; i < col.size(); i++){
                if(col.get(i) != 0){
                    if(cart.peek() == col.get(i)) {
                		cart.pop();
                		answer += 2;
                	}else{
                        cart.push(col.get(i));
