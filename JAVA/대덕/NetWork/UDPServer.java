...
/*
 	UDP방식 : 비연결 지향, 신뢰성이 없다. 데이터의 순서보장X 그렇지만 TCP방식보다 빠르다.
 	
 	- TCP방식의 경우에는 스트림을 이용해서 송수신을 하고, UDP방식의 경우에는 데이터그램을 이용해서 송수신을 한다.
	...
	public static void main(String[] args) {
		try {
			...
			while(true) {
				...
				// 수신용 패킷 객체 생성
				// ==> 데이터가 저장될 byte형 배열, 배열의 길이를 이용하여 DatagramPacket객체를 생성
				inpacket = new DatagramPacket(bMsg, bMsg.length);
				
				// 데이터 수신하기 ==> receive()메서드 호출
				// receive() : 데이터가 올 때까지 기다린다. 수신된 데이터의 패킷정보는 지정한 패킷객체변수에 저장된다.
...
//				String msg = new String(bMsg, 0, inpacket.getLength(), "utf-8");
				...
				// 상대방에게 메세지 보내기(수신받은 데이터 그대로 송신)
				
				// 송신할 메세지를 byte배열로 변환
				byte[] sendMsg = msg.getBytes("utf-8");
				
				// 송신용 패킷객체 생성
				//  ==> 전송할 데이터가 저장된 byte배열, 전송할 자료의 길이, 상대방 주소정보, 포트번호 필요
