...
public class JdbcTest01 {
	...
	/*
	 	...
	 	1. 드라이버 로딩 ==> 라이브러리를 사용할 수 있게 메모리로 읽어 들이는 작업
	 		...
	 		(JDBC API 버전 4이상에서는 getConnection()메서드에서 자동으로 로딩해주기 때문에 생략할 수 있다. 그렇지만 생략하지 않고 사용할 예정)
	 	2. DB에 접속하기 ==> 접속이 완료되면 Connection객체가 반환된다. DriverManager.getConnection()메서드를 이용한다.
	 	3. 질의 ==> SQL문장을 DB서버로 보내서 결과를 얻어온다. (Statement객체 또는 PreparedStatement객체를 이용하여 작업한다.)
	 	4. 결과 처리 ==> 질의 결과를 받아서 원하는 작업을 수행한다.
	 	  1) SQL문이 select문일 경우
	 	  	==> Select한 결과가 ResultSet객체에 저장되어 반환된다.
	 	  2) SQL문이 select문이 아닐 경우
	 	  	==> 정수값 즉, 해당 명령이 실행에 성공한 레코드 수를 반환
	...
	public static void main(String[] args) {
		...
		try {
			...
			conn = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "SSK03", "java"); // url, user, password 입력
...
//			String sql = "select * from lprod";
			...
			while(rs.next()) { // rs.next() ==> ResultSet객체의 데이터를 가리키는 포인터를 다음번째 위치로 이동시키고 그 곳에 데이터가 있으면 true, 없으면 false반환
